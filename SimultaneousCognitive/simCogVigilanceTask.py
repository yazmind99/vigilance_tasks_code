...
def play_video():
    video = visual.MovieStim(win, filename=r'C:\Users\yesse\OneDrive\Documents\GitHub\Test\vigilance_tasks_code\SimultaneousCognitive\MeSinging.mp4', flipVert=False, flipHoriz=False, loop=False)
    ...
    while video.status != visual.FINISHED:
        ...
        # Exit the loop once the video duration has elapsed
        if core.getTime() - start_time >= video.duration:
            video.stop()
            break
...
text_screen1 = visual.TextStim(win, text='Insert gaming survey.', height=1, color='black', pos=(0,5))
text_screen2 = visual.TextStim(win, text='Insert video.', height = 1, color='black', pos=(0,5))
...
''' For Yessenia bc she keeps forgetting:
      ...
      1 count = 2 seconds '''
max_count_practice = 1 # 5 minutes 
max_count_experiment = 2 # 30 minutes 
...
while current <= 5:
    if current == 0:
        ...
        gaming_survey = "https://run.pavlovia.org/pavlovia/survey-2024.2.0/?surveyId=9dc52710-5381-420c-aa6d-6699d31ff3c5"
        webbrowser.open(gaming_survey)
